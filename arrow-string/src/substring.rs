...
pub fn substring_by_char<OffsetSize: OffsetSizeTrait>(
    ...
    // Optimization: if the start offset is non-negative, we can avoid calculating
    // the character count for each string, which requires a full pass.
    if start >= 0 {
        ...
        array.iter().for_each(|val| {
            if let Some(val) = val {
                // With a negative start, we must calculate the character count.
