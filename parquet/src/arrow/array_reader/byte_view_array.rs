...
impl ByteViewArrayDecoderPlain {
    ...
    pub fn read(&mut self, output: &mut ViewBuffer, len: usize) -> Result<usize> {
        ...
        // In order to accelerate the validation of utf8, we copy the string data into a contiguous buffer
        // So that we can validate the entire buffer in one go.
        // This is inspired by the implementation of `ByteViewArrayDecoderDelta::read`.
        ...
        while self.offset < self.buf.len() && read != to_read {
            ...
            if let Some(validation_buffer) = &mut utf8_validation_buffer {
                validation_buffer.extend_from_slice(&buf[start_offset..end_offset]);
            }
        ...
        if let Some(validation_buffer) = &utf8_validation_buffer {
            check_valid_utf8(validation_buffer)?;
        }
